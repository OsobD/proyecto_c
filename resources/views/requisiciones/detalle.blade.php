...
<div class="container mx-auto px-4 py-6">
    @php
        ...
        if ($tipo === 'salida') {
            $requisicion = \App\Models\Salida::with(['persona', 'bodega', 'detalles.producto', 'detalles.lote'])
                ->findOrFail($id);
            $esConsumible = false;
            $tipoNombre = 'Salida - Productos No Consumibles';
            $tipoColor = 'blue';
        } else {
            $requisicion = \App\Models\Traslado::with(['tarjetaResponsabilidad.persona', 'bodegaOrigen', 'detalles.producto', 'detalles.lote'])
                ->findOrFail($id);
            $esConsumible = true;
            $tipoNombre = 'Traslado - Productos Consumibles';
            $tipoColor = 'amber';
        }

        $persona = $tipo === 'salida'
            ? ($requisicion->persona ? $requisicion->persona->nombres . ' ' . $requisicion->persona->apellidos : 'N/A')
            : ($requisicion->tarjetaResponsabilidad && $requisicion->tarjetaResponsabilidad->persona
                ? $requisicion->tarjetaResponsabilidad->persona->nombres . ' ' . $requisicion->tarjetaResponsabilidad->persona->apellidos
                : 'N/A');

        $bodega = $tipo === 'salida'
            ? ($requisicion->bodega ? $requisicion->bodega->nombre : 'N/A')
            : ($requisicion->bodegaOrigen ? $requisicion->bodegaOrigen->nombre : 'N/A');

        $correlativo = $tipo === 'salida' ? $requisicion->ubicacion : ($requisicion->no_requisicion ?? $requisicion->correlativo);
        $descripcion = $tipo === 'salida' ? $requisicion->descripcion : $requisicion->observaciones;
    @endphp
