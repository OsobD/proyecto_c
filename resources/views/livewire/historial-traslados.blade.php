    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-semibold text-gray-800">
                Traslados encontrados: <span class="text-blue-600">{{ count($trasladosFiltrados) }}</span>
            </h2>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full bg-white">
                ...
                <tbody class="text-gray-600 text-sm font-light">
                    @forelse($trasladosFiltrados as $traslado)
                        <tr class="border-b border-gray-200 hover:bg-gray-50 {{ !$traslado['activo'] ? 'opacity-50' : '' }}">
                            ...
                            <td class="py-3 px-6 text-center">
                                <div class="flex items-center justify-center gap-4">
                                    <button
                                        wire:click="verDetalle({{ $traslado['id'] }})"
                                    ...
                                    @if($traslado['activo'])
                                        <button
                                            wire:click="editarTraslado({{ $traslado['id'] }})"
                                            class="text-gray-600 hover:text-gray-800 font-medium px-2">
                                            Editar
                                        </button>
                                        <span class="text-gray-300">|</span>
                                        <button
                                            wire:click="desactivarTraslado({{ $traslado['id'] }})"
                                            onclick="return confirm('¿Está seguro de desactivar este traslado?')"
                                            class="text-red-600 hover:text-red-800 font-medium px-2">
                                            Desactivar
                                        </button>
                                    @else
                                        <button
                                            wire:click="activarTraslado({{ $traslado['id'] }})"
    ...
    @if($showModalEditar)
        <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" wire:click="closeModalEditar">
            <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white" wire:click.stop>
                <div class="mt-3">
                    <h3 class="text-lg font-medium leading-6 text-gray-900 mb-4">Editar Traslado</h3>
                    <p class="text-sm text-gray-500 mb-4">
                        Esta funcionalidad requiere permisos de supervisor.
                    </p>
                    <div class="bg-gray-100 p-4 rounded mb-4">
                        <p class="text-sm"><strong>Tipo:</strong> {{ $trasladoSeleccionado['tipo'] ?? '' }}</p>
                        <p class="text-sm"><strong>Correlativo:</strong> {{ $trasladoSeleccionado['correlativo'] ?? '' }}</p>
                        <p class="text-sm"><strong>Origen:</strong> {{ $trasladoSeleccionado['origen'] ?? '' }}</p>
                        <p class="text-sm"><strong>Destino:</strong> {{ $trasladoSeleccionado['destino'] ?? '' }}</p>
                    </div>
