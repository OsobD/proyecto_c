{{--
    Vista principal del Módulo de Compras (Hub).
    Esta vista actúa como un panel de control, mostrando estadísticas clave,
    accesos directos a funciones importantes y una lista de las compras más recientes.
    Utiliza datos del componente `ComprasHub`.
--}}
<div>
    ...
    {{-- Muestra tarjetas con información clave como total de compras, monto y proveedores. --}}
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {{-- Tarjeta: Compras este Mes --}}
    ...
    {{-- Enlaces a las páginas más importantes del módulo de compras. --}}
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16 mt-12">
        <a href="{{ route('compras.nueva') }}" class="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow border-t-4 border-blue-600">
            <div class="flex items-center gap-4">
                <div class="bg-blue-100 p-3 rounded-lg"><svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg></div>
                <div><h3 class="text-base font-semibold text-gray-800">Nueva Compra</h3><p class="text-xs text-gray-600 mt-0.5">Registrar una nueva compra</p></div>
            </div>
        </a>
        <a href="{{ route('compras.historial') }}" class="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow border-t-4 border-green-600">
            <div class="flex items-center gap-4">
                ...
                <div><h3 class="text-base font-semibold text-gray-800">Historial</h3><p class="text-xs text-gray-600 mt-0.5">Ver todas las compras</p></div>
            </div>
        </a>
        <a href="{{ route('proveedores') }}" class="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow border-t-4 border-purple-600">
            <div class="flex items-center gap-4">
                ...
                <div><h3 class="text-base font-semibold text-gray-800">Proveedores</h3><p class="text-xs text-gray-600 mt-0.5">Gestionar proveedores</p></div>
            </div>
        </a>
    </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="{{ route('compras.historial') }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">Ver todas →</a>
        </div>
        {{-- Tabla con las últimas compras registradas. --}}
                    ...
                    @foreach($comprasRecientes as $compra)
                        <tr class="border-b border-gray-200 hover:bg-gray-50">
                            ...
                            <td class="py-3 px-6 text-center">
                                {{-- Condicional para mostrar badge de estado (Completada/Pendiente) --}}
                                @if($compra['estado'] === 'Completada')
                                    <span class="bg-green-200 text-green-800 py-1 px-3 rounded-full text-xs font-semibold">Completada</span>
                                @else
                                    <span class="bg-yellow-200 text-yellow-800 py-1 px-3 rounded-full text-xs font-semibold">Pendiente</span>
                                @endif
                            </td>
                            <td class="py-3 px-6 text-center">
